...
def _get_local_machine_ip() -> str:
    """
    Lấy IP của interface mạng thực sự của máy (không phải loopback 127.x).
    Dùng trick connect UDP tới 8.8.8.8 — không gửi gói tin thật,
    chỉ để OS chọn interface ra ngoài và lấy IP của interface đó.
    """
    try:
...
async def detect_public_ip_async(timeout: int = 5) -> str:
    # Ưu tiên 1: IP local của máy (interface thực tế, không qua NAT)
    local_ip = _get_local_machine_ip()
    if local_ip:
        return local_ip

    # Ưu tiên 2: fallback về IP public qua OpenDNS (khi không lấy được IP local)
    try:
        od = aiodns.DNSResolver(nameservers=["208.67.222.222"], timeout=timeout, tries=1)
        answers = await asyncio.wait_for(od.query("myip.opendns.com", "A"), timeout=timeout)
        ips = [getattr(a, "host", "") for a in answers if getattr(a, "host", "")]
