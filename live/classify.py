...
#     Trả về SERVER_DEAD | WAF_BLOCK | LIVE.
#     """
...
#             original_label,
#             STATUS_LIVE,
#             f"Đối chứng Worker: HTTP {proxy_status} → LIVE",
...
#         original_label,
#         STATUS_BLOCKED,
#         f"Bị ISP chặn (đối chứng HTTP {proxy_status} — web vẫn phản hồi ngoài VN)",
#         proxy_final_url=proxy_final_url,
#         proxy_http_status=proxy_status,
#         local_rcode=local_rcode,
#         local_ips=local_ips,
...
#             original_label,
#             STATUS_LIVE,
#             f"Trang phản hồi bình thường (HTTP {status_code})",
#             final_url,
#             http_code,
#             redirect_chain,
#             local_rcode,
...
#         original_label,
#         STATUS_BLOCKED,
#         f"Phản hồi bất thường (HTTP {status_code})",
#         final_url,
#         http_code,
#         redirect_chain,
#         local_rcode,
...
from .constants import (
    ...
    STATUS_DEAD,
    STATUS_LEAKED,
    STATUS_LIVE,   # thêm hằng số này trong constants.py
)
from .dns import (
    detect_dns_sinkhole,
    resolve_a_and_aaaa,
    ...
    try:
        ...
        if isinstance(data, dict):
            if "status" in data:
                return data
            if "error" in data:
                return {"error": f"worker_msg: {data['error']}"}
        return {"error": f"worker_http_{res.status_code}"}
    except Exception as e:
        return {"error": f"worker_fail: {str(e)}"}
    ...
    *,
    ...
    if kind == "LIVE":
        return _build_worker_row(
            original_label,
            STATUS_LEAKED,
            f"Đối chứng Worker: HTTP {proxy_status} → LEAKED",
            proxy_final_url=proxy_final_url,
            proxy_http_status=proxy_status,
            local_rcode=local_rcode,
            local_ips=local_ips,
